...
import { DDRoundStats, HistoryRecord, GameStatusModalInfo } from '../types/DDtypes';
...
export const useDDGameLogic = (baseNumber: number) => {
  ...
  const [selectedBetAmount, setSelectedBetAmount] = useState(0); // 选择的投注金额
  // 合并游戏状态弹窗相关状态
  ...
  const continueGame = useCallback(async () => {
    ...
    const params = {
      betNumber: selectedBetAmount,
      eventNum: ddRoundStats.round,
    ...
    setDDRoundStats(newStats);
    // 关闭确认对话框
    setConfirmModalVisible(false);
    // 设置游戏状态为已完成
    setGameStatus('finished');
  }, [banker, player, ddRoundStats, currentChoice, gameNumber, roundId, selectedBetAmount]);
  ...
  useEffect(() => {
    const handleGameLogic = async () => {
      if (gameStatus === 'finished') {
        console.log('游戏状态为finished,轮次信息', ddRoundStats);
        // 重置押注
        setBanker(0);
        setPlayer(0);
        // 重置游戏状态为等待押注
